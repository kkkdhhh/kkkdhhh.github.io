...
var GHSyncSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Remote URL").setDesc("").addText((text) => text.setPlaceholder("").setValue(this.plugin.settings.remoteURL).onChange(async (value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Git Directory").setDesc("Select a subdirectory within your Obsidian Vault to use for Git operations. The default is the 'Obsidian Vault' directory.").addDropdown(async (dropdown) => {
      dropdown.addOption("", "Obsidian Vault").addOptions(
        (await this.app.vault.adapter.list("")).folders.map((filePath) => this.app.vault.getAbstractFileByPath(filePath)).filter((file) => file instanceof import_obsidian.TFolder).reduce((acc, dir) => {
          acc[dir.path] = dir.path;
          return acc;
        }, {})
      ).setValue(this.plugin.settings.gitDirectory).onChange(async (value) => {
